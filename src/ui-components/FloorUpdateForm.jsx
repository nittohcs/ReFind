...
import { getFloor } from "../graphql/queries";
import { updateFloor } from "../graphql/mutations";
const client = generateClient();
export default function FloorUpdateForm(props) {
  const {
    id: idProp,
    floor: floorModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = floorRecord
      ? { ...initialValues, ...floorRecord }
      : initialValues;
    setTenantId(cleanValues.tenantId);
    setName(cleanValues.name);
    setImagePath(cleanValues.imagePath);
    setImageWidth(cleanValues.imageWidth);
    setImageHeight(cleanValues.imageHeight);
    setErrors({});
  };
  const [floorRecord, setFloorRecord] = React.useState(floorModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getFloor.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getFloor
        : floorModelProp;
      setFloorRecord(record);
    };
    queryData();
  }, [idProp, floorModelProp]);
  React.useEffect(resetStateValues, [floorRecord]);
  const validations = {
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateFloor.replaceAll("__typename", ""),
            variables: {
              input: {
                id: floorRecord.id,
                ...modelFields,
      ...
      {...getOverrideProps(overrides, "FloorUpdateForm")}
      {...rest}
    >
      <TextField
        label="Tenant id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || floorModelProp)}
